...
const VideoTile = ({stream , name , isLocal=false , audioEnabled=true , videoEnabled=true}) => {
  ...
  return (
    <div className="relative w-full h-full min-h-50 bg-slate-900 rounded-2xl overflow-hidden 
    border border-slate-800 shadow-md flex items-center justify-center group">
        {/* video element */}
        ...
        {`w-full h-full object-cover transition-opacity duration-300 
            ...
            <div className='absolute bottom-3 left-3 right-3 flex items-center justify-between z-20 pointer-events-none'>
                <div className='flex items-center gap-2 bg-slate-900/80 backdrop-blur-md px-3 py-1.5 
                rounded-xl border border-white/20 text-xs font-medium text-white shadow-md'>
                    <span>
                        {name} {isLocal? "(You)":""}
                    </span>
                    {!audioEnabled && (
