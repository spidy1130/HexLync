import { Calendar1Icon, CalendarIcon, MessageSquareIcon, UserIcon, UsersIcon } from 'lucide-react';
import React from 'react'

const SessionCard = ({session,onOpenDetails,onRejoin}) => {
    const isEnded=session.status==="ended";
  return (
    <div className='bg-white/70 backdrop-blur rounded-3xl p-6 transition-all felx flex-col
    justify-between space-y-5 border border-slate-100/60 shadow-xs'>
        <div className='space-y-3'>
            <div className='flex items-center justify-between'>
                ...
                <span className={`text-xs font-semibold px-2.5 py-1 rounded-full flex 
                    items-center gap-1.5 ${isEnded?"bg-slate-500/5 text-slate-500":"bg-emerald-500/5 text-emerald-500"}`}>
      ...
      {/* Stats Row */}
      <div className='grid grid-cols-2 gap-3 border-t border-slate-300/30'>
            <div className='flex items-center gap-2 text-xs bg-slate-500/5 p-2.5 rounded-xl'>
                <UsersIcon className='w-4 h-4 text-primary'/>
                <span>
                    <strong className='font-semibold text-slate-900'>{session.participants?.length||0} Participants</strong>
                </span>
            </div>
            ...
            <div className='flex items-center gap-2 text-xs bg-slate-500/5 p-2.5 rounded-xl'>
                ...
                <span>
                    <strong className='font-semibold text-slate-900'>{session.messages?.length||0} Messages</strong>
                </span>
            </div>
      </div>
      {/* Actions */}
      <div className='flex items-center justify-between gap-3 pt-2'>
        ...
        {!isEnded && (
            ...
            px-4 rounded-full text-xs transition-all shadow-xs cursor-pointer text-center'>
                Re-Join
            </button>
        )}
